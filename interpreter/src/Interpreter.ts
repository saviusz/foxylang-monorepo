...
import { Addition, Multiplication, NumberValue, Subtraction, Value } from "./models/Value";

export function Interpret(tokens: Token[]): number {
    const tree = GenSyntaxTree(tokens);
    console.log(tree.getString());
    return tree.getValue()
}

function GenSyntaxTree(tokens: Token[]): Value {
    const stack: Value[] = []
    ...
    if (operation != "") throw new Error("Unmached operation")
    return stack[0]
}